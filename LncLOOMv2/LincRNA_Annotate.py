...
def parse_mir_species_table():
    SpeciesFile = os.path.join(os.path.dirname(__file__), 'src', 'species100.html')
...
def annotate_TargetScan(filename,project_name,combined_kmers_dict,sequences,uracil):
    ...
    FamilyFile = os.path.join(os.path.dirname(__file__), 'src', 'miR_Family_Info.txt')
...
def run_blat(outdir,project_name,default_head,default_seq,genome):
    blat_path = os.path.join(os.path.dirname(__file__), 'src', 'blat')
    #genome_path = os.path.join(os.path.dirname(__file__), 'src', 'hg19.fa')
    num_of_exons = 0
    exon_sizes = []
    exon_pos_seq = []
    exon_pos_chrom = []
    ...
    try:
        ...
        w= open(outdir+'/'+project_name+'/Run_Files/query_'+default_head+'.fa','w') #set buffereing to false so that file is written immediately 
        ...
        subprocess.call([blat_path,genome,outdir+'/'+project_name+'/Run_Files/query_'+default_head+'.fa', outdir+'/'+project_name+'/Run_Files/'+default_head+".psl"])
    ...
    try:
        ...
        if len(psl_output)==5:
            ...
            while not hit_found and i>=0:
                ...
                i-=1
            if not hit_found:
                print("ERROR!!! EXON STRUCTURE OF QUERY SEQUENCE DOES NOT MATCH BLAT RESULTS: UPLOAD CUSTOMIZED BEDFILE FOR eCLIP ANALYSIS...\n")
                aligned = False
...
def parse_bedFile(bedfile,sequence):
    ...
    try:
        ...
        exon_sizes = list(map(int,top_hit[10].strip().strip(',').split(',')))
        relative_exon_pos = list(map(int,top_hit[11].strip().strip(',\n').split(',')))
        exon_pos_seq = []
        exon_pos_chrom = []
        start_in_seq = 0
