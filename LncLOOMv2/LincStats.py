...
def remove_gaps(shuffled_msa,mp,iteration):
    ...
    w = open(outdir+"/"+project_name+"/Run_Files/ShuffledMSA_"+str(mp)+'_'+str(iteration+1)+".fasta",'w')
...
def build_random_sequence_set(mp,iteration):
    ...
    for seq in eligable_sequences:
        ...
        w = open(outdir+"/"+project_name+"/Run_Files/Random_"+str(mp)+'_'+str(iteration+1)+".fasta",'w')
    ...
    try:
        ...
        if mafft:
            ...
            if iterations%multiprocess==0:
                sub_iterations = [int(iterations/multiprocess) for i in range(multiprocess)]
            else:
                sub_iterations = [int(iterations/multiprocess) for i in range(multiprocess)]
