...
def run_mafft_msa(outdir,project_name):
    ...
    out_file = open(outdir+"/"+project_name+"/Run_Files/Mafft.fasta","w")
    try:
...
def exlcudeSimilarSequences(outdir,project_name,sequences,headers,seq_lengths,intron_indices,mafft,similarity):
    ...
    if mafft and similarity<100:
        ...
        #rewrite mafft file
        if similar_indexes:
            w = open(outdir+"/"+project_name+"/Run_Files/Mafft.fasta","w")
    ...
    w= open(outdir+"/"+project_name+"/Run_Files/Sequences.fasta",'w') #set buffereing to false so that file is written immediately 
...
def prune_graph_level(G,level):
    ...
    remove_nodes =[n for n in F.nodes() if F.nodes[n]['layer']>level]
...
def prune_graph_level_plus(G,level):
    ...
    for n in [x for x in F.nodes() if F.nodes[x]['layer']>1]:
...
def hasOverlap(path1,path2,G):
    ...
    if len(path2)>len(path1):
        ...
        r = len(path2)
    for i in range(r):
        end1 = G.nodes[path1[i][0]]['end']
        start2 = G.nodes[path2[i][0]]['location']
        if start2<=end1:
            overlap=True
    return overlap
...
def solve_lpp(G,solver):
    ...
    graph_kmers = list(dict.fromkeys([G.nodes[x]['seq'] for x in G.nodes()]))
    ...
    if len(graph_kmers)==1:
        ...
        graph_layers = list(dict.fromkeys([G.nodes[x]['layer'] for x in G.nodes()]))
        graph_layers.sort()
        ...
        for i in nodes_pruned:
            ...
            if int(G.nodes[i]['layer'])!=graph_layers[-1] and int(G.nodes[i]['layer'])!=graph_layers[0]:
            ...
            if float(G.nodes[i]['layer'])!=graph_layers[-1]:

                #iterate through the successors of that node:            
                for r in G.successors(i):

                    for x in [e for e in graph_egdes if G.nodes[i]['layer']==G.nodes[e[0]]['layer'] if G.nodes[i]['end']>=G.nodes[e[0]]['location'] and G.nodes[i]['location']<G.nodes[e[0]]['location']]:
                        if G.nodes[i]['end']-G.nodes[x[0]]['location']!=G.nodes[r]['end']-G.nodes[x[1]]['location']:
                            prob += lpSum(vars[m] for m in [(i,r),x])<=1


                    for x in [e for e in graph_egdes if G.nodes[i]['layer']==G.nodes[e[0]]['layer'] if G.nodes[r]['end']>=G.nodes[e[1]]['location'] and G.nodes[r]['location']<G.nodes[e[1]]['location']]:
                        if G.nodes[i]['end']-G.nodes[x[0]]['location']!=G.nodes[r]['end']-G.nodes[x[1]]['location']:
                            prob += lpSum(vars[m] for m in [(i,r),x])<=1
        ...
        if solver=='GUROBI':
            try:
                ...
                try:
                    ...
                    print("Check Gurobi License and ensure Gurobi has been added to your path")
            ...
            try:
                ...
                try:
                    ...
                    print("Note, to ensure pulp is installed for python3 use the command: python3 -m pip install -U git+https://github.com/coin-or/pulp")
        ...
        for v in prob.variables():
            ...
            if round(v.varValue)==1.0:
                ...
                for attribute in G.nodes[node1].keys():
                    F.nodes[node1][attribute]=G.nodes[node1][attribute]
                    F.nodes[node2][attribute]=G.nodes[node2][attribute]
    else:
        nodes_pruned = list(G.nodes())
        #Get all edges of the graph
        graph_egdes=list(G.edges())
        ...
        for i in nodes_pruned:
            ...
            if float(G.nodes[i]['layer'])!=graph_layers[-1]:
                ...
                for r in G.successors(i):
                    ...
                    all_edges_layer = [e for e in graph_egdes if G.nodes[e[0]]['layer']==G.nodes[i]['layer'] if e!=(i,r)]
                    constraint_set = [e for e in all_edges_layer if G.nodes[i]['location']<G.nodes[e[0]]['location'] and G.nodes[r]['location']>G.nodes[e[1]]['location']]
                    constraint_kmers = [G.nodes[e[0]]['seq'] for e in constraint_set]
                    constraint_kmers.append(G.nodes[i]['seq'])
...
def getComplexPaths(G):
    ...
    #Get first and last layers of graph:
    top_nodes = [x for x in F.nodes() if F.nodes[x]['layer']==1]
    UF = F.to_undirected()
    ...
    while len(top_nodes)>0:
        ...
        repeat = False
        kmer = F.nodes[top_node]['seq']
        kmer_edges = []
        

        
        tn_aux_neighbors = list(nx.single_source_shortest_path(UF,top_node).keys())#gets a list of all auxially neighbors with a path to the top_node
        tn_aux_neighbors.sort(key=lambda x: UF.nodes[x]['layer'])
...
def split_graph(graph,simple_paths,CN,LongCN):
    ...
    graph_layers = list(dict.fromkeys([graph.nodes[x]['layer'] for x in graph.nodes()]))
    ...
    all_simple_paths.sort(key=lambda x: graph.nodes[x[0][0]]['location'])
    ...
    for cluster in overlapped:
        ...
        for edge in first_path:
            if edge[0] in graph.nodes():
                end = graph.nodes[edge[0]]['location']-1
                layer = graph.nodes[edge[0]]['layer']
        ...
        for edge in last_path:  
            if edge[0] in graph.nodes():
                start = graph.nodes[edge[0]]['end']+1
                #startCN = graph.nodes[edge[0]]['location']+1
                layer = graph.nodes[edge[0]]['layer']
    ...
    for i in range(number_of_subgraphs-1):
        ...
        for layer in split_indices:
            ...
            G.add_nodes_from([x for x in graph.nodes() if graph.nodes[x]['layer']==layer and graph.nodes[x]['location']>=s and graph.nodes[x]['end']<=e])
            #G.add_nodes_from([x for x in CN.nodes() if CN.nodes[x]['layer']==layer and CN.nodes[x]['location']>=sc and CN.nodes[x]['location']<=ec])
            #G.add_nodes_from([x for x in LongCN.nodes() if LongCN.nodes[x]['layer']==layer and LongCN.nodes[x]['location']>=sc and LongCN.nodes[x]['location']<=ec])
            cp.add_nodes_from([x for x in CN.nodes() if CN.nodes[x]['layer']==layer and CN.nodes[x]['location']>=sc and CN.nodes[x]['location']<=ec])
            longer_cp.add_nodes_from([x for x in LongCN.nodes() if LongCN.nodes[x]['layer']==layer and LongCN.nodes[x]['location']>=sc and LongCN.nodes[x]['location']<=ec])
        ...
        for x in cp.nodes():
            for attribute in CN.nodes[x].keys():
                cp.nodes[x][attribute]=CN.nodes[x][attribute]
 
        #add edges that continue past the current level
        last_cp = [x for x in cp.nodes() if cp.nodes[x]['layer']==layer]
        while last_cp:
            for x in last_cp:
                successors = list(CN.successors(x))
                for s in successors:
                    cp.add_node(s,layer=CN.nodes[s]['layer'],location=CN.nodes[s]['location'],seq=CN.nodes[s]['seq'],end=CN.nodes[s]['end'],posx=CN.nodes[s]['posx'])
                    cp.add_edge(x,s)
            layer+=1          
            last_cp = [x for x in cp.nodes() if cp.nodes[x]['layer']==layer]
        ...
        for x in longer_cp.nodes():
            for attribute in LongCN.nodes[x].keys():
                longer_cp.nodes[x][attribute]=LongCN.nodes[x][attribute]
 
        #add edges that continue past the current level
        last_cp = [x for x in longer_cp.nodes() if longer_cp.nodes[x]['layer']==layer]
        while last_cp:
            for x in last_cp:
                successors = list(LongCN.successors(x))
                for s in successors:
                    longer_cp.add_node(s,layer=LongCN.nodes[s]['layer'],location=LongCN.nodes[s]['location'],seq=LongCN.nodes[s]['seq'],end=LongCN.nodes[s]['end'],posx=LongCN.nodes[s]['posx'])
    ...
    for layer in split_indices:
        s = split_indices[layer][0][i]
        sc = split_indices_CN[layer][0][i]
        G.add_nodes_from([x for x in graph.nodes() if graph.nodes[x]['layer']==layer and graph.nodes[x]['location']>=s])
        #G.add_nodes_from([x for x in CN.nodes() if CN.nodes[x]['layer']==layer and CN.nodes[x]['location']>=sc])
        #G.add_nodes_from([x for x in LongCN.nodes() if LongCN.nodes[x]['layer']==layer and LongCN.nodes[x]['location']>=sc])
        cp.add_nodes_from([x for x in CN.nodes() if CN.nodes[x]['layer']==layer and CN.nodes[x]['location']>=sc])
        longer_cp.add_nodes_from([x for x in LongCN.nodes() if LongCN.nodes[x]['layer']==layer and LongCN.nodes[x]['location']>=sc])
    G.add_edges_from([x for x in graph.edges() if G.has_node(x[0]) and G.has_node(x[1])])
    cp.add_edges_from([x for x in CN.edges() if cp.has_node(x[0]) and cp.has_node(x[1])])
    longer_cp.add_edges_from([x for x in LongCN.edges() if longer_cp.has_node(x[0]) and longer_cp.has_node(x[1])])
    ...
    for sub in subgraph_array:
        G=sub[0]
        for x in G.nodes():
            for attribute in graph.nodes[x].keys():
                G.nodes[x][attribute]=graph.nodes[x][attribute]
        '''for x in [n for n in G.nodes() if n in graph.nodes()]:
            for attribute in graph.nodes[x].keys():
                G.nodes[x][attribute]=graph.nodes[x][attribute]
        for x in [n for n in G.nodes() if n in LongCN.nodes()]:
            for attribute in LongCN.nodes[x].keys():
                G.nodes[x][attribute]=LongCN.nodes[x][attribute]'''
...
def get_neighbourhoods(G):
    ...
    top_nodes = [x for x in F.nodes() if F.nodes[x]['layer']==1]
    top_nodes.sort(key=lambda x: F.nodes[x]['location'])
    ...
    while len(top_nodes)>0:
        ...
        tn_aux_neighbors.sort(key=lambda x: (F.nodes[x]['layer'],F.nodes[x]['location']))
    ...
    while i<total:
        neighbours = [paths[i]]
        tandems = [x for x in paths[i] if G.nodes[x]['layer']==1]
        end = G.nodes[tandems[-1]]['end']
        depth = G.nodes[paths[i][-1]]['layer']
        j = i+1
        break_overlap = True
        while j<total and break_overlap:
            if end>=G.nodes[paths[j][0]]['location']:
                neighbours.append(paths[j])
                tandems = [x for x in paths[j] if G.nodes[x]['layer']==1]
                if G.nodes[tandems[-1]]['end']>end:
                    end = G.nodes[tandems[-1]]['end']
                if G.nodes[paths[j][-1]]['layer']>depth:
                    depth = G.nodes[paths[j][-1]]['layer']
                i+=1
            else:
                break_overlap = False 
            j+=1
    ...
    for nh in neighbourhoods:
        ...
        for path in paths:
            path_depth = G.nodes[path[-1]]['layer']
            #np = []
            for n in path:
                layer = G.nodes[n]['layer']
                nodes_per_layer[layer].append(n)
                #np.append((G.nodes[n]['layer'],G.nodes[n]['location'],G.nodes[n]['end'],G.nodes[n]['seq']))
        ...
        for layer in nodes_per_layer:
            ...
            nodes.sort(key=lambda x: G.nodes[x]['end'])
            limit2 = G.nodes[nodes[-1]]['end']
            nodes.sort(key=lambda x: (G.nodes[x]['location'],len(G.nodes[x]['seq'])))
            limit1 = G.nodes[nodes[0]]['location']
            combined_nodes = []

            i = 0
            total = len(nodes)
            while i<total:
                cn = G.nodes[nodes[i]]['seq']
                start = G.nodes[nodes[i]]['location']
                end = G.nodes[nodes[i]]['end'] 
                j = i+1
                break_overlap = True
                while j<total and break_overlap:
                    if end>=G.nodes[nodes[j]]['location'] and G.nodes[nodes[j]]['end']>end:
                        cn = cn[0:G.nodes[nodes[j]]['location']-start]+G.nodes[nodes[j]]['seq']
                        end = G.nodes[nodes[j]]['end']
                        i+=1
                    elif end>=G.nodes[nodes[j]]['location'] and G.nodes[nodes[j]]['end']<=end:
                        i+=1
                    else:
                        break_overlap = False 
...
def getNonIntersectingPaths(F):
    ...
    if len(G.nodes())>0:
        layerID = sorted(list(dict.fromkeys([G.nodes[x]['layer'] for x in G.nodes()])))[0] 
    while len(G.nodes())>0:
        layerID = sorted(list(dict.fromkeys([G.nodes[x]['layer'] for x in G.nodes()])))[0] 
        top_node = [x for x in G.nodes() if G.nodes[x]['layer']==layerID][0]
        kmer = G.nodes[top_node]['seq']
        kmer_edges = []
...
def split_at_current_level(LOOM_Level,G,C,LC):
    ...
    for kmer in simple_paths:
        for path in simple_paths[kmer]:
            all_simple_paths.append(path)
    #all_simple_paths.sort(key=lambda x: LOOM_Level.nodes[x[0][0]]['location'])
    all_simple_paths.sort(key=lambda x: int(x[0][0].split('_')[1]))
    total_paths = len(all_simple_paths)
    overlapped = []
    i=0
    ...
    graph_layers = list(dict.fromkeys([LOOM_Level.nodes[x]['layer'] for x in LOOM_Level.nodes()]))
    graph_layers.sort()
    split_indices = {l:[[1],[]] for l in graph_layers}
    split_indices_CN = {l:[[1],[]] for l in graph_layers}
    number_of_subgraphs = len(overlapped)+1
    ...
    for cluster in overlapped:
        ...
        edge = first_path[0]
        end = LOOM_Level.nodes[edge[0]]['location']-1
        layer = LOOM_Level.nodes[edge[0]]['layer']
        split_indices[layer][1].append(end)
        split_indices_CN[layer][0].append(end+1)
        
        for edge in first_path:
            end = LOOM_Level.nodes[edge[1]]['location']-1
            layer = LOOM_Level.nodes[edge[1]]['layer']
            split_indices[layer][1].append(end)
            split_indices_CN[layer][0].append(end+1)

        edge = last_path[0]
        #start = LOOM_Level.nodes[edge[0]]['location']+1
        start = LOOM_Level.nodes[edge[0]]['location']
        layer = LOOM_Level.nodes[edge[0]]['layer']
        split_indices[layer][0].append(start)
            
        for edge in last_path:  
            #start = LOOM_Level.nodes[edge[1]]['location']+1
            start = LOOM_Level.nodes[edge[1]]['location']
            layer = LOOM_Level.nodes[edge[1]]['layer']
            split_indices[layer][0].append(start)
    ...
    for i in range(number_of_subgraphs-1):
        ...
        for layer in split_indices:
            ...
            e = split_indices[layer][1][i]
            sc = split_indices_CN[layer][0][i]
            S.add_nodes_from([x for x in G.nodes() if G.nodes[x]['layer']==layer and G.nodes[x]['location']>=s and G.nodes[x]['location']<=e])
            #S.add_nodes_from([x for x in C.nodes() if C.nodes[x]['layer']==layer and C.nodes[x]['location']>=sc and C.nodes[x]['location']<=e])
            #S.add_nodes_from([x for x in LC.nodes() if LC.nodes[x]['layer']==layer and LC.nodes[x]['location']>=sc and LC.nodes[x]['location']<=e])
            cp.add_nodes_from([x for x in C.nodes() if C.nodes[x]['layer']==layer and C.nodes[x]['location']>=sc and C.nodes[x]['location']<=e])
            lcp.add_nodes_from([x for x in LC.nodes() if LC.nodes[x]['layer']==layer and LC.nodes[x]['location']>=sc and LC.nodes[x]['location']<=e])
        ...
        for x in cp.nodes():
            for attribute in C.nodes[x].keys():
                cp.nodes[x][attribute]=C.nodes[x][attribute]
        ...
        while last_cp:
            for x in last_cp:
                ...
                for s in successors:
                    cp.add_node(s,layer=C.nodes[s]['layer'],location=C.nodes[s]['location'],seq=C.nodes[s]['seq'],end=C.nodes[s]['end'],posx=C.nodes[s]['posx'])
                    cp.add_edge(x,s)
            layer+=1          
            last_cp = [x for x in cp.nodes() if cp.nodes[x]['layer']==layer]
        ...
        for x in lcp.nodes():
            for attribute in LC.nodes[x].keys():
                lcp.nodes[x][attribute]=LC.nodes[x][attribute]
 
        #add edges that continue past the current level
        last_cp = [x for x in lcp.nodes() if lcp.nodes[x]['layer']==layer]
        while last_cp:
            for x in last_cp:
                successors = list(LC.successors(x))
                for s in successors:
                    lcp.add_node(s,layer=LC.nodes[s]['layer'],location=LC.nodes[s]['location'],seq=LC.nodes[s]['seq'],end=LC.nodes[s]['end'],posx=LC.nodes[s]['posx'])
                    lcp.add_edge(x,s)
            layer+=1          
            last_cp = [x for x in lcp.nodes() if lcp.nodes[x]['layer']==layer]
    ...
    for layer in split_indices:
        s = split_indices[layer][0][i]
        sc = split_indices_CN[layer][0][i]
        S.add_nodes_from([x for x in G.nodes() if G.nodes[x]['layer']==layer and G.nodes[x]['location']>=s])
        #S.add_nodes_from([x for x in C.nodes() if C.nodes[x]['layer']==layer and C.nodes[x]['location']>=sc])
        #S.add_nodes_from([x for x in LC.nodes() if LC.nodes[x]['layer']==layer and LC.nodes[x]['location']>=sc])
        cp.add_nodes_from([x for x in C.nodes() if C.nodes[x]['layer']==layer and C.nodes[x]['location']>=sc])
        lcp.add_nodes_from([x for x in LC.nodes() if LC.nodes[x]['layer']==layer and LC.nodes[x]['location']>=sc])
    S.add_edges_from([x for x in G.edges() if S.has_node(x[0]) and S.has_node(x[1])])
    cp.add_edges_from([x for x in C.edges() if cp.has_node(x[0]) and cp.has_node(x[1])])
    lcp.add_edges_from([x for x in LC.edges() if lcp.has_node(x[0]) and lcp.has_node(x[1])])
    ...
    for sub in subgraph_array:
        ...
        for x in [n for n in S.nodes() if n in G.nodes()]:
            for attribute in G.nodes[x].keys():
                S.nodes[x][attribute]=G.nodes[x][attribute]
        '''for x in [n for n in S.nodes() if n in LC.nodes()]:
            for attribute in LC.nodes[x].keys():
                S.nodes[x][attribute]=LC.nodes[x][attribute]
        for x in [n for n in S.nodes() if n in C.nodes()]:
            for attribute in C.nodes[x].keys():
                S.nodes[x][attribute]=C.nodes[x][attribute]'''    
    ...
    if len(G.edges())+len(C.edges())>maxedges:
        ...
        if len(graph_kmers)==1:
            ...
            for i in graph_nodes:
                ...
                if int(G.nodes[i]['layer'])<graph_layers[level-1] and int(G.nodes[i]['layer'])!=graph_layers[0]:
                    prob += lpSum([vars[(i,a)] for a in G.successors(i)])<= 100*lpSum([vars[(b,i)] for b in G.predecessors(i)])          
                    prob += lpSum([vars[(a,i)] for a in G.predecessors(i)])<= 100*lpSum([vars[(i,b)] for b in G.successors(i)])


                #if the node is not in the last layer
                if float(G.nodes[i]['layer'])<graph_layers[level-1]:
            ...
            #Define the constraints
            special_kmers = list(dict.fromkeys([C.nodes[x]['seq'] for x in C.nodes() if C.nodes[x]['layer']==1]))
    
            for kmer in special_kmers:
                special_edges = [x for x in G.edges() if G.nodes[x[0]]['seq']== kmer and G.nodes[x[0]]['layer']== 1]
                if special_edges:
...
def get_shallow_overlaps(graphS,graphD):
    ...
    for n in graphD.nodes():
        overlap = [x for x in graphS.nodes() if graphS.nodes[x]['layer']==graphD.nodes[n]['layer'] and graphS.nodes[x]['location']<=graphD.nodes[n]['location'] and graphD.nodes[n]['location']<=graphS.nodes[x]['end']]
        overlapping_nodes.extend(overlap)
        overlap = [x for x in graphS.nodes() if graphS.nodes[x]['layer']==graphD.nodes[n]['layer'] and graphS.nodes[x]['location']>=graphD.nodes[n]['location'] and graphS.nodes[x]['location']<=graphD.nodes[n]['end']]
        overlapping_nodes.extend(overlap)
            ...
            for seg_of_seg in segments_of_segment:
                ...
                if len(C.nodes())>0 and len(PrunedLC.nodes())>0:
                    ...
                    if len(C.edges())+len(PrunedLC.edges())>maxedges:
                        ...
                        #ed = [e for e in OptimisedComplex.edges() if OptimisedComplex.nodes[e[0]]['seq']=='TGTACATT']
                ...
                if len(F.nodes())>0: #Proceed with solving at current level
                    ...
                    if len(OptimisedGraph.nodes())>0:
                        ...
                        last_layer = [x for x in OptimisedSimple.nodes() if OptimisedSimple.nodes[x]['layer']>level]
                        ...
                        last_layer = [x for x in graph.nodes() if graph.nodes[x]['layer']>=level]
...
def remove_intersecting_edges(F,LOOM):
    ...
    graph_layers = list(dict.fromkeys([F.nodes[x]['layer'] for x in F.nodes()]))
    graph_layers.extend(list(dict.fromkeys([LOOM.nodes[x]['layer'] for x in LOOM.nodes()])))
    ...
    for node in F.nodes():
        ...
        for ns in node_successors:
            dict_of_edges[F.nodes[node]['layer']][(node,ns)]={}
            dict_of_edges[F.nodes[node]['layer']][(node,ns)]['location'] = F.nodes[node]['location']
            dict_of_edges[F.nodes[node]['layer']][(node,ns)]['slocation'] = F.nodes[ns]['location']
            dict_of_edges[F.nodes[node]['layer']][(node,ns)]['end'] = F.nodes[node]['end']
            dict_of_edges[F.nodes[node]['layer']][(node,ns)]['send'] = F.nodes[ns]['end']

            #dict_of_edges[F.nodes[node]['layer']][F.nodes[node]['location']]['edge']=(node,ns)

    remove_edges = []           
    constraint_edges = LOOM.edges()
    for ce in constraint_edges:
        remove_edges = []
        layer = LOOM.nodes[ce[0]]['layer']
        location1 = LOOM.nodes[ce[0]]['location']
        location2 = LOOM.nodes[ce[1]]['location']
        end1 = LOOM.nodes[ce[0]]['end']
        end2 = LOOM.nodes[ce[1]]['end']
...
def cut_and_prune(kmers_len,LG):
    ...
    for x in all_nodes:
        sequence = LG.nodes[x]['seq']
        start = LG.nodes[x]['location']
        layer = LG.nodes[x]['layer']
...
def get_graphs_per_level(LOOM,min_depth):
    LevelGraphs = {}
    total_levels = sorted(list(dict.fromkeys([LOOM.nodes[x]['layer'] for x in LOOM.nodes()])))
    if total_levels:
...
def get_kmers_per_layer(All_Graphs,number_of_layers):
    ...
    #LOOM_layers = list(dict.fromkeys([LOOM.nodes[x]['layer'] for x in LOOM.nodes()]))
    All_Combined_Kmers =  {i+1:[] for i in range(number_of_layers)}
    Checked_Combined_Kmers =  {i+1:[] for i in range(number_of_layers)}
    levels = list(All_Graphs.keys())
    levels.sort(reverse=True)

    for level in levels:
        G = All_Graphs[level]
        G_layers = list(dict.fromkeys([G.nodes[x]['layer'] for x in G.nodes()]))
        for layer in G_layers:
            nodes_in_layer = [(G.nodes[x]['location'],G.nodes[x]['end'],G.nodes[x]['seq']) for x in G.nodes() if G.nodes[x]['layer']==layer]
...
def calculate_modules(Graph_at_levels,number_of_layers):
    ...
    levels = list(Graph_at_levels.keys())
    levels.sort(reverse=True)
    ...
    for kmer in paths:
        for p in paths[kmer]:
            for edge in p:
                for x in edge:
                    graphA.add_node(x)
                    for attribute in graphB.nodes[x].keys():
                        graphA.nodes[x][attribute] = graphB.nodes[x][attribute]
...
def calculateMissing5Prime(F,total_seq,tolerance):
    ...
    for i in range(total_seq):
        layer = i+1
        positions_of_nodes_in_layer =[F.nodes[x]['location'] for x in F.nodes() if F.nodes[x]['layer']==layer]
    ...
    if num_nodes%2!=0:
        median = float(firstNodeLocations[int(num_nodes/2)])
    else:
        median = (firstNodeLocations[int(num_nodes/2)]+firstNodeLocations[int(num_nodes/2)-1])/2.0
...
def calculateMissing3Prime(F,total_seq,tolerance):
    ...
    for i in range(total_seq):
        layer = i+1
        positions_of_nodes_in_layer =[F.nodes[x]['posx'] for x in F.nodes() if F.nodes[x]['layer']==layer]       
        positions_of_nodes_in_layer.sort()
    ...
    if num_nodes%2!=0:
        median = float(lastNodeLocations[int(num_nodes/2)])
    else:
        median = (lastNodeLocations[int(num_nodes/2)]+lastNodeLocations[int(num_nodes/2)-1])/2.0
...
def cutFor3PrimeIteration(F,sequences):
    ...
    for i,seq in enumerate(sequences):
        layer = i+1
        positions_of_nodes_in_layer =[F.nodes[x]['end'] for x in F.nodes() if F.nodes[x]['layer']==layer]
        positions_of_nodes_in_layer.sort()
        if positions_of_nodes_in_layer: #If the layer contains any nodes     
            lastNode = positions_of_nodes_in_layer[-1]
...
def build_LOOM(sequences,seq_lengths,kmers_len,stopw,hsps,solver,prune,min_depth,shorttandem,maxedges,noconstraints):
    ...
    #print([x for x in LOOM.edges() if LOOM.nodes[x[0]]['layer']==1])
...
def get_extenstion_barriers_start(SG):
    ...
    graph_layers = list(dict.fromkeys([SG.nodes[x]['layer'] for x in SG.nodes()]))
    graph_layers.sort()
    barriers = {}
    for i in range(len(graph_layers)-1):
        start_edges1 = [SG.nodes[x[0]]['location'] for x in SG.edges() if SG.nodes[x[0]]['layer']==graph_layers[i]]
        start_edges2 = [SG.nodes[x[1]]['location'] for x in SG.edges() if SG.nodes[x[0]]['layer']==graph_layers[i]]
...
def get_extenstion_barriers_end(SG):
    ...
    barriers = {}
    for i in range(len(graph_layers)-1):
        end_edges1 = [SG.nodes[x[0]]['end'] for x in SG.edges() if SG.nodes[x[0]]['layer']==graph_layers[i]]
        end_edges2 = [SG.nodes[x[1]]['end'] for x in SG.edges() if SG.nodes[x[0]]['layer']==graph_layers[i]]
...
def refine_LOOM(SG,sequences,min_depth):
    ...
    for layer in graph_layers:
        #connected_bases_end[layer] = [SG.nodes[x]['end'] for x in SG.nodes() if SG.nodes[x]['layer']==layer]
        #connected_bases_start[layer] = [SG.nodes[x]['location'] for x in SG.nodes() if SG.nodes[x]['layer']==layer]
        #for pos in node_positions:
            #connected_bases[layer].extend(range(pos[0],pos[1]+1))
        nodes_in_layer = [(SG.nodes[x]['location'],SG.nodes[x]['end'],SG.nodes[x]['seq']) for x in SG.nodes() if SG.nodes[x]['layer']==layer]
        nodes_in_layer = sorted(nodes_in_layer,key=lambda x: (x[0],-len(x[2])))
    ...
    for kmer in simple_paths:
        for path in simple_paths[kmer]:
            all_simple_paths.append(path)
    all_simple_paths.sort(key=lambda x: SG.nodes[x[0][0]]['location'])
    ...
    all_complex_paths.sort(key=lambda x: SG.nodes[x[0][0]]['location'])


    #process simple paths
    simple_extend = nx.DiGraph()
    for path in all_simple_paths:
        path.sort(key=lambda x: SG.nodes[x[0]]['layer'])
        connect = True
        for edge in path:
            start1 = SG.nodes[edge[0]]['location']
            start2 = SG.nodes[edge[1]]['location']
            layer1 = SG.nodes[edge[0]]['layer']
            layer2 = SG.nodes[edge[1]]['layer']
            ...
            if (start1==1 or start2==1) and Allowed:
                ...
                end1 = SG.nodes[edge[0]]['end']
                end2 = SG.nodes[edge[1]]['end']
                ...
                if base1==base2:
                    if start1-1 not in connected_bases_end[layer1] and start2-1 not in connected_bases_end[layer2]:
                        seq1 = base1+SG.nodes[edge[0]]['seq']
                        seq2 = base2+SG.nodes[edge[1]]['seq']
                        posx1 = SG.nodes[edge[0]]['posx']
                        posx2 = SG.nodes[edge[1]]['posx']                        
    ...
    for node in simple_extend.nodes():
        layer = simple_extend.nodes[node]['layer']
        start = simple_extend.nodes[node]['location']
            ...
            if (end1==len(sequences[layer1-1]) or end2==len(sequences[layer2-1])) and Allowed:
                ...
                if base1==base2:
                    if end1+1 not in connected_bases_start[layer1] and end2+1 not in connected_bases_start[layer2]:
                        seq1 = SG.nodes[edge[0]]['seq']+base1
                        seq2 = SG.nodes[edge[1]]['seq']+base2
                        start1 = SG.nodes[edge[0]]['location']
                        start2 = SG.nodes[edge[1]]['location']
    ...
    for node in simple_extend.nodes():
        layer = simple_extend.nodes[node]['layer']
        end = simple_extend.nodes[node]['end']
    ...
    for path in all_complex_paths:
        ...
        nodes_per_layer = {}
        for edge in [e for e in path if SG.nodes[e[0]]['layer']==1]:
            node = edge[0]
            node2 = edge[1]
            layer1 = SG.nodes[edge[0]]['layer']
            layer2 = SG.nodes[edge[1]]['layer']
            start1 = SG.nodes[node]['location']
            if start1!=1:
                end1 = SG.nodes[node]['end']
                base1 = sequences[layer1-1][start1-2]          
                if start1-1 not in connected_bases_end[layer1]:
                    seq1 = base1+SG.nodes[node]['seq']
                    posx1 = SG.nodes[node]['posx']

                    start2 = SG.nodes[node2]['location']
                    ...
                    if start2!=1 and Allowed:
                        end2 = SG.nodes[node2]['end']
                        base2 = sequences[layer2-1][start2-2]          
                        if start2-1 not in connected_bases_end[layer2] and base1==base2:
                            seq2 = base2+SG.nodes[node2]['seq']
                            posx2 = SG.nodes[node2]['posx']
                            ...
                            C.add_node(node,layer=layer1,location=start1,seq=SG.nodes[node]['seq'],end=end1,posx=posx1)
                            C.add_node(node2,layer=layer2,location=start2,seq=SG.nodes[node2]['seq'],end=end2,posx=posx2)
        ...
        while i<last_layer and local_connect:
            local_connect = False
            nodes1 = [x for x in C.nodes() if C.nodes[x]['layer']==i]
            ...
            for edge in edges2:
                node1 = edge[0]
                node2 = edge[1]
                start1 = SG.nodes[node1]['location']
                end1 = SG.nodes[node1]['end']
                base1 = sequences[layer1-1][start1-2]          
                seq1 = base1+SG.nodes[node]['seq']
                posx1 = SG.nodes[node]['posx']       
                start2 = SG.nodes[node2]['location']
                ...
                if start2!=1 and Allowed:
                    ...
                    if start2-1 not in connected_bases_end[layer2] and seq1==seq2:
                        ...
                        E.add_node(str(layer2)+'_'+str(start2-1)+'_'+seq2,layer=layer2,location = start2-1,seq=seq2,end=end2,posx=posx2)
                        E.add_edge(str(layer1)+'_'+str(start1-1)+'_'+seq1,str(layer2)+'_'+str(start2-1)+'_'+seq2)
                        #for cn in [x for x in C.nodes() if C.nodes[x]['layer']==layer1]:
                        C.add_node(node2,layer=layer2,location = start2,seq=SG.nodes[node2]['seq'],end=end2,posx=posx2)
            ...
            if not local_connect:
                ...
                for j in range(i,last_layer):
                    nodes1 = [x for x in C.nodes() if C.nodes[x]['layer']==j]
                    edges2 = [x for x in path if x[0] in nodes1]
                    for edge in edges2:
                        node = edge[1]
                        pn = edge[0]
                        C.add_node(node,layer=j+1,location=SG.nodes[node]['location'],seq=SG.nodes[node]['seq'],end=SG.nodes[node]['end'],posx=SG.nodes[node]['posx'])
                        C.add_edge(pn,node)
            i+=1
        ...
        if len(E.nodes())>0:
            ...
            depth_nodes = {E.nodes[x]['seq']:[] for x in E.nodes()}
            if len(depth_nodes)==1:
                ...
                for x in E.nodes():
                    depth_nodes[E.nodes[x]['seq']].append(E.nodes[x]['layer'])
                ...
                to_remove = [x for x in E.nodes if E.nodes[x]['seq']!=kmer]
                to_removeC = [str(E.nodes[x]['layer'])+'_'+str(E.nodes[x]['location']+1)+'_'+kmer[1:] for x in to_remove]
    ...
    if len(EP.nodes)>0: 
        ...
        for kmer in complex_paths:
            for path in complex_paths[kmer]:
                all_complex_paths.append(path)
        all_complex_paths.sort(key=lambda x: EP.nodes[x[0][0]]['location'])
    
        for node in EP.nodes():
            layer = EP.nodes[node]['layer']
            start = EP.nodes[node]['location']
            if start+1 in connected_bases_start[layer]:
                connected_bases_start[layer].append(start)
        ...
        for path in all_complex_paths:
            ...
            C = nx.DiGraph()
            nodes_per_layer = {}
            for edge in [e for e in path if EP.nodes[e[0]]['layer']==1]:
                node = edge[0]
                node2 = edge[1]
                layer1 = EP.nodes[edge[0]]['layer']
                layer2 = EP.nodes[edge[1]]['layer']
                start1 = EP.nodes[node]['location']
                end1 = EP.nodes[node]['end']
                if end1!=len(sequences[layer1-1]):
                    base1 = sequences[layer1-1][end1]          
                    if end1+1 not in connected_bases_end[layer1]:
                        seq1 = EP.nodes[node]['seq']+base1
                        posx1 = EP.nodes[node]['posx']
    
                        start2 = EP.nodes[node2]['location']
                        end2 = EP.nodes[node2]['end']
                        ...
                        if end2!=len(sequences[layer2-1]) and Allowed:                        
                            ...
                            if end2+1 not in connected_bases_start[layer2] and base1==base2:
                                seq2 = EP.nodes[node2]['seq']+base2
                                posx2 = EP.nodes[node2]['posx']
                                ...
                                C.add_node(node,layer=layer1,location=start1,seq=EP.nodes[node]['seq'],end=end1,posx=posx1)
                                C.add_node(node2,layer=layer2,location=start2,seq=EP.nodes[node2]['seq'],end=end2,posx=posx2)
                                C.add_edge(node,node2)      
                                local_connect = True
            ...
            while i<last_layer and local_connect:
                ...
                for edge in edges2:
                    node1 = edge[0]
                    node2 = edge[1]
                    start1 = EP.nodes[node1]['location']
                    end1 = EP.nodes[node1]['end']
                    base1 = sequences[layer1-1][end1]          
                    seq1 = EP.nodes[node]['seq']+base1
                    posx1 = EP.nodes[node]['posx']       
                    start2 = EP.nodes[node2]['location']
                ...
                if not local_connect:
                    ...
                    for j in range(i,last_layer):
                        ...
                        for edge in edges2:
                            node = edge[1]
                            pn = edge[0]
                            C.add_node(node,layer=j+1,location=EP.nodes[node]['location'],seq=EP.nodes[node]['seq'],end=EP.nodes[node]['end'],posx=EP.nodes[node]['posx'])
                            C.add_edge(pn,node)
                i+=1
            ...
            if len(E.nodes())>0:
                ...
                if len(depth_nodes)==1:
                    E = remove_intersecting_edges(E,SG)
                    E = prune_graph_level_plus(E,min_depth)
                    #to_removeEP = [x for x in EP.nodes() if str(EP.nodes[x]['layer'])+'_'+str(EP.nodes[x]['location']+1)+'_'+EP.nodes[x]['seq'][1:] not in C.nodes()]
                    ...
                    to_remove = [x for x in E.nodes if E.nodes[x]['seq']!=kmer]
                    to_removeC = [str(E.nodes[x]['layer'])+'_'+str(E.nodes[x]['location']+1)+'_'+kmer[0:-1] for x in to_remove]
                  
                    E.remove_nodes_from(to_remove)
                    C.remove_nodes_from(to_removeC)
                    
                    #to_removeEP = [x for x in EP.nodes() if str(EP.nodes[x]['layer'])+'_'+str(EP.nodes[x]['location']+1)+'_'+kmer[1:] not in C.nodes()]
                    #EP.remove_nodes_from(to_removeEP)
...
def start_lncLOOM(sequences,seq_lengths,kmers_len,stopw,hsps,solver,prune,min_depth,shorttandem,maxedges,noconstraints,tolerance):
    ...
    e = [x for x in LOOM.edges if LOOM.nodes[x[0]]['layer']==2]
        ...
        if 1 not in remove5prime and len(remove5prime)>0:
            ...
            if len(LOOM_5Prime.nodes())>0:
                ...
                for edge in LOOM_5Prime.edges():
                    node1 = edge[0]
                    node2 = edge[1]
                    layer1 = layer_ids[LOOM_5Prime.nodes[node1]['layer']]
                    layer2 = layer_ids[LOOM_5Prime.nodes[node2]['layer']]
                    label1 = str(layer1)+'_'+str(LOOM_5Prime.nodes[node1]['location'])+'_'+LOOM_5Prime.nodes[node1]['seq']
                    label2 = str(layer2)+'_'+str(LOOM_5Prime.nodes[node2]['location'])+'_'+LOOM_5Prime.nodes[node2]['seq']
                    
                    Corrected5.add_node(label1,layer=layer1,location=LOOM_5Prime.nodes[node1]['location'],end=LOOM_5Prime.nodes[node1]['end'],seq=LOOM_5Prime.nodes[node1]['seq'],posx=(LOOM_5Prime.nodes[node1]['end'])/float(len(sequences[layer1-1])))
                    Corrected5.add_node(label2,layer=layer2,location=LOOM_5Prime.nodes[node2]['location'],end=LOOM_5Prime.nodes[node2]['end'],seq=LOOM_5Prime.nodes[node2]['seq'],posx=(LOOM_5Prime.nodes[node2]['end'])/float(len(sequences[layer2-1])))
                ...
                for kmer in paths5:
                    for path in paths5[kmer]:
                        path.sort(key=lambda x: Corrected5.nodes[x[0]]['layer'])
                        depth = Corrected5.nodes[path[-1][1]]['layer']
                        if depth<min_depth:
                            for edge in path:
                                to_remove.append(edge[0])
                                to_remove.append(edge[1])
                        for l in range(depth,min_depth-1,-1):
                            ...
                            prune_layers = [x for x in LOOM_at_Levels[l].nodes() if LOOM_at_Levels[l].nodes[x]['layer']>l]
        ...
        if 1 not in remove3prime and len(remove3prime)>0:
            ...
            if len(LOOM_3Prime.nodes())>0:
                ...
                for edge in LOOM_3Prime.edges():
                    node1 = edge[0]
                    node2 = edge[1]
                    layer1 = layer_ids[LOOM_3Prime.nodes[node1]['layer']]
                    layer2 = layer_ids[LOOM_3Prime.nodes[node2]['layer']]
                    label1 = str(layer1)+'_'+str(LOOM_3Prime.nodes[node1]['location']+end_of_last_nodes[layer1-1])+'_'+LOOM_3Prime.nodes[node1]['seq']
                    label2 = str(layer2)+'_'+str(LOOM_3Prime.nodes[node2]['location']+end_of_last_nodes[layer2-1])+'_'+LOOM_3Prime.nodes[node2]['seq']
                    
                    Corrected3.add_node(label1,layer=layer1,location=LOOM_3Prime.nodes[node1]['location']+end_of_last_nodes[layer1-1],end=LOOM_3Prime.nodes[node1]['end']+end_of_last_nodes[layer1-1],seq=LOOM_3Prime.nodes[node1]['seq'],posx=(LOOM_3Prime.nodes[node1]['end']+end_of_last_nodes[layer1-1])/float(len(sequences[layer1-1])))
                    Corrected3.add_node(label2,layer=layer2,location=LOOM_3Prime.nodes[node2]['location']+end_of_last_nodes[layer2-1],end=LOOM_3Prime.nodes[node2]['end']+end_of_last_nodes[layer2-1],seq=LOOM_3Prime.nodes[node2]['seq'],posx=(LOOM_3Prime.nodes[node2]['end']+end_of_last_nodes[layer2-1])/float(len(sequences[layer2-1])))
                ...
                for kmer in paths3:
                    for path in paths3[kmer]:
                        path.sort(key=lambda x: Corrected3.nodes[x[0]]['layer'])
                        depth = Corrected3.nodes[path[-1][1]]['layer']
                        if depth<min_depth:
                            for edge in path:
                                to_remove.append(edge[0])
                                to_remove.append(edge[1])
